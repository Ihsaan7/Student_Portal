...
export default function ChatPage() {
  ...
  return (
    <DashboardLayout currentPage="/chat">
      <div className="max-w-7xl mx-auto h-[calc(100vh-160px)] px-2 sm:px-4 w-full">
        <div className="rounded-lg shadow-sm border h-full flex flex-col w-full max-w-full" style={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          {/* Compact Header */}
          <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b" style={{ borderColor: 'hsl(var(--border))' }}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
              <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                <div className="flex-shrink-0">
                  <h1 className="text-lg sm:text-xl font-bold" style={{ color: 'hsl(var(--card-foreground))' }}>Student Support</h1>
                  <p className="text-xs sm:text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>{queries.length} query{queries.length !== 1 ? 's' : ''}</p>
                </div>
                {queries.length > 0 && (
                  <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-xs sm:text-sm">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="font-medium" style={{ color: 'hsl(var(--success))' }}>
                        <span className="hidden sm:inline">{getQueryStats().solved} Solved</span>
                        <span className="sm:hidden">{getQueryStats().solved}✓</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="font-medium" style={{ color: 'hsl(var(--warning))' }}>
                        <span className="hidden sm:inline">{getQueryStats().pending} Pending</span>
                        <span className="sm:hidden">{getQueryStats().pending}⏳</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: 'hsl(var(--destructive))' }}></div>
                      <span className="font-medium" style={{ color: 'hsl(var(--destructive))' }}>
                        <span className="hidden sm:inline">{getQueryStats().unsolved} Unsolved</span>
                        <span className="sm:hidden">{getQueryStats().unsolved}❌</span>
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="flex items-center space-x-2 flex-shrink-0">
                ...
                <span className="text-xs sm:text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>
                  <span className="hidden sm:inline">Support Online</span>
          ...
          {/* Queries List */}
          <div className="flex-1 overflow-y-auto px-2 sm:px-4 py-3 w-full">
            {queries.length === 0 ? (
              ...
              <div className="space-y-3">
                {queries.map((query) => (
                  <div key={query.id} className="rounded-lg p-3 sm:p-4 border w-full max-w-full" style={{ backgroundColor: 'hsl(var(--muted))', borderColor: 'hsl(var(--border))' }}>
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 space-y-2 sm:space-y-0">
                        <div className="flex items-center space-x-2 min-w-0 flex-1">
                          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'hsl(var(--primary) / 0.1)' }}>
                            <span className="font-semibold text-xs sm:text-sm" style={{ color: 'hsl(var(--primary))' }}>
                              {userProfile?.name?.charAt(0) || user?.email?.charAt(0)}
                            </span>
                          </div>
                          <div className="min-w-0 flex-1">
                            <h3 className="font-medium text-xs sm:text-sm truncate" style={{ color: 'hsl(var(--card-foreground))' }}>{userProfile?.name || user?.email}</h3>
                            <p className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>{formatTime(query.created_at)}</p>
                          </div>
                        </div>
                      <span className={`px-2 py-1 text-xs font-medium rounded-full border flex-shrink-0 ${getStatusBadge(query.status)}`}>
                    ...
                    <div className="mb-3">
                      {query.query_type === 'file' ? (
                        <div>
                          ...
                          <div className="rounded-lg p-3 sm:p-4 border w-full max-w-full" style={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
                            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                              <svg className="w-6 h-6 sm:w-8 sm:h-8 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{ color: 'hsl(var(--muted-foreground))' }}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                              </svg>
                              <div className="flex-1 min-w-0">
                                <p className="font-medium text-sm sm:text-base truncate" style={{ color: 'hsl(var(--card-foreground))' }}>{query.file_name}</p>
                                <p className="text-xs sm:text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>{formatFileSize(query.file_size)}</p>
                              </div>
                              <a
                                href={query.file_url}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="px-3 py-1.5 rounded-md text-xs sm:text-sm transition flex-shrink-0 text-center"
                        ...
                        <div>
                          {editingQuery === query.id ? (
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-2 sm:space-y-0">
                              <p className="flex-1 text-sm sm:text-base break-words" style={{ color: 'hsl(var(--card-foreground))' }}>{query.query_text}</p>
                              {query.status === 'pending' && (
                                <button
                                  onClick={() => startEditing(query)}
                                  className="sm:ml-2 px-2 py-1 text-xs rounded transition flex-shrink-0 self-start"
          ...
          {/* Query Input */}
          <div className="px-2 sm:px-4 py-3 border-t w-full" style={{ borderColor: 'hsl(var(--border))', backgroundColor: 'hsl(var(--muted))' }}>
            <form onSubmit={handleSubmitQuery}>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mb-2">
                <input
                  ...
                  className="flex-1 px-3 sm:px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 text-sm w-full"
                  style={{ borderColor: 'hsl(var(--border))', backgroundColor: 'hsl(var(--background))', color: 'hsl(var(--foreground))', '--tw-ring-color': 'hsl(var(--ring))' }}
                ...
                <div className="flex space-x-2 sm:space-x-0">
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none px-3 py-2.5 transition border rounded-lg"
                  ...
                  <button
                    type="submit"
                    className="flex-1 sm:flex-none px-4 sm:px-5 py-2.5 rounded-lg transition disabled:opacity-50 font-medium text-sm"
              ...
              <p className="text-xs break-words" style={{ color: 'hsl(var(--muted-foreground))' }}>
                <span className="hidden sm:inline">Attach files (images, PDFs, Word docs - max 5MB) • Support responds within 24 hours</span>
                <span className="sm:hidden">Max 5MB files • 24h response</span>
              </p>
            </form>
          </div>
        </div>
